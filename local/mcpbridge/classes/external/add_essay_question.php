...
/**
 * External function to add an essay question to a quiz.
 *
...
/**
 * External function: add an essay (free-text, manually graded) question to a quiz.
 *
 * @package    local_mcpbridge
 * @copyright  2026 Namaa Alhawary <namaa.alhawary@htu.edu.jo>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class add_essay_question extends external_api {
    /**
    ...
    /**
     * Add the essay question to the quiz.
     *
     ...
     * @param string $graderinfo Notes for graders (HTML).
     * @param string $responsetemplate Template pre-filled in the answer box.
     * @param int $responselines Height of the answer box in lines.
     * @param int $attachments Allowed attachments (0-3, -1 unlimited).
    ...
    public static function execute($quizcmid, $name, $questiontext, $graderinfo = '',
        ...
        $form->responsefieldlines  = max(1, (int) $params['responselines']);
        $form->attachments         = (int) $params['attachments'];
        $form->attachmentsrequired = 0;
        $form->maxbytes            = 0;
        $form->graderinfo          = ['text' => $params['graderinfo'], 'format' => FORMAT_HTML];
        $form->responsetemplate    = ['text' => $params['responsetemplate'], 'format' => FORMAT_HTML];
